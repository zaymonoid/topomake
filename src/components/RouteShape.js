...
import { beginInteractionAtom, setPointNoCommitAtom, insertPointAtom, deletePointAtom, } from "../state/actions";
...
export function RouteShape({ route, imageWidth, imageHeight, svgRef }) {
    ...
    const onHandleContextMenu = (e, index) => {
        if (isDrawing)
            return;
    ...
    return (_jsxs("g", { children: [pathD && (_jsx("path", { d: pathD, stroke: "transparent", strokeWidth: hitWidth, fill: "none", style: { cursor: isSelected ? (isDrawing ? "default" : "copy") : "pointer" }, onClick: onLineClick })), pathD && (_jsxs(_Fragment, { children: [_jsx("path", { d: pathD, stroke: "#fff", strokeWidth: haloWidth, fill: "none", strokeLinecap: "round", strokeLinejoin: "round", pointerEvents: "none" }), _jsx("path", { d: pathD, stroke: color, strokeWidth: lineWidth, fill: "none", strokeLinecap: "round", strokeLinejoin: "round", pointerEvents: "none" })] })), end && pixelPoints.length >= 2 && (_jsxs(_Fragment, { children: [_jsx("circle", { cx: end.x, cy: end.y, r: endR, fill: "none", stroke: "#fff", strokeWidth: haloWidth, pointerEvents: "none" }), _jsx("circle", { cx: end.x, cy: end.y, r: endR, fill: "none", stroke: color, strokeWidth: lineWidth, pointerEvents: "none" })] })), start && (_jsxs("g", { style: { cursor: "pointer" }, onClick: (e) => { e.stopPropagation(); setSelectedId(route.id); }, children: [_jsx("circle", { cx: start.x, cy: start.y, r: startR + haloWidth / 2, fill: "#fff" }), _jsx("circle", { cx: start.x, cy: start.y, r: startR, fill: color }), _jsx("text", { x: start.x, y: start.y, fontSize: startFontSize, fill: numColor, textAnchor: "middle", dominantBaseline: "central", fontWeight: "700", pointerEvents: "none", style: { userSelect: "none" }, children: route.number })] })), isSelected &&
                pixelPoints.map((p, i) => (_jsx("circle", { cx: p.x, cy: p.y, r: handleR, fill: "#fff", stroke: "#000", strokeWidth: lineWidth * 0.5, opacity: 0.85, style: { cursor: "grab" }, onPointerDown: (e) => onHandleDown(e, i), onPointerMove: onHandleMove, onPointerUp: onHandleUp, onPointerCancel: onHandleUp, onContextMenu: (e) => onHandleContextMenu(e, i) }, i)))] }));
}
